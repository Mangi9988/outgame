...
{
    // new -> 생성자가 호출되면서 인스턴스
    // MonoBehaviour -> 게임 오브젝트이므로 -> Instantiate

    public readonly string ID;
    public readonly DateTime StartDate;      // 이 출석(이벤트)을 언제부터 시작할 것인가?
    ...
    public Attendance(string id, DateTime startDate,DateTime lastAttendanceDate, int dayCount)
    {
        ...
        if (DayCount <= 0)
        {
            throw new Exception("출석 일수는 0보다 작을 수 없습니다.");
        }
        ...
        {
            throw new Exception("출석일은 이벤트 시작일보다 작을 수 없습니다.");
        }
